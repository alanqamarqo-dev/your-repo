...
class AGL: # type: ignore
    ...
    def _initialize_safety_systems(self):
        ...
        # provide a safe default target file for the rollback mechanism inside artifacts/
        try:
            rb_target = str(Path(__file__).parent / 'artifacts' / 'fusion_weights.json')
            # ensure artifacts dir exists (no overwrite of repo files)
            os.makedirs(str(Path(__file__).parent / 'artifacts'), exist_ok=True)
        except Exception:
            rb_target = 'artifacts/fusion_weights.json'
        self.safety_systems['rollback_mechanism'] = RollbackMechanism(target_file=rb_target)
    ...
    def process_complex_problem(self, problem_description, context=None):
        ...
        # Normalize task for engines: prefer a short textual task representation
        try:
            if isinstance(formatted_task, str):
                engine_task = formatted_task
            elif isinstance(formatted_task, dict):
                engine_task = formatted_task.get('main_task') or formatted_task.get('text') or ''
                if not engine_task:
                    import json as _json
                    engine_task = _json.dumps(formatted_task, ensure_ascii=False)
            else:
                engine_task = str(formatted_task)
        except Exception:
            engine_task = str(formatted_task)
        ...
        # 3. معالجة متوازية بواسطة المحركات (نمرّر تمثيلاً نصياً مبسّطاً للمحركات)
        partial_solutions = self._parallel_processing(engine_allocation, engine_task)
    ...
    def _parallel_processing(self, engines, task):
        ...
        for engine_name in engines:
            engine = self.core_engines.get(engine_name)
            if engine is None:
                solutions[engine_name] = {'error': 'engine_not_found'}
                continue

            try:
                # Preferred interface
                if hasattr(engine, 'process_task') and callable(getattr(engine, 'process_task')):
                    try:
                        solution = engine.process_task(task)
                    except Exception:
                        # Some engines expect a text input; try fallback
                        solution = engine.process_task(str(task))
                else:
                    # Try alternative common method names
                    alt_fn = None
                    for name in ('process', 'analyze', 'run', 'infer', 'execute'):
                        if hasattr(engine, name) and callable(getattr(engine, name)):
                            alt_fn = getattr(engine, name)
                            break
                    if alt_fn is not None:
                        try:
                            solution = alt_fn(task)
                        except Exception:
                            solution = alt_fn(str(task))
                    else:
                        raise AttributeError(f"{engine.__class__.__name__} missing process_task and alternative interfaces")
