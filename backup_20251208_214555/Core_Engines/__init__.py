...
def bootstrap_register_all_engines(
    ...
    for name, spec in ENGINE_SPECS.items():
        # Respect max_seconds timeout if requested: stop attempting further
        # engine imports once elapsed time exceeds the allowed window. This
        # prevents very long test-collection delays when optional heavy
        # engines (large ML libs, external deps) are present but slow to
        # import/initialize.
        try:
            if max_seconds is not None and (time.time() - start_time) > max_seconds:
                skipped_remaining = {k: "skipped due to bootstrap timeout" for k in list(ENGINE_SPECS.keys())[list(ENGINE_SPECS.keys()).index(name):]}
                skipped.update(skipped_remaining)
                logger.warning(f"Bootstrap time exceeded {max_seconds}s; skipping remaining {len(skipped_remaining)} engines")
                break
        except Exception:
            # If any odd error occurs while checking time, continue gracefully
            pass
...
# إنشاء ENGINE_REGISTRY العام وتعبئته تلقائياً
ENGINE_REGISTRY: Dict[str, Any] = {}

# تعبئة ENGINE_REGISTRY تلقائياً عند الاستيراد (اختياري)
try:
    bootstrap_register_all_engines(
        registry=ENGINE_REGISTRY,
        allow_optional=True,
        verbose=False,
        max_seconds=15  # timeout سريع للاستيراد
    )
except Exception as _bootstrap_err:
    # في حالة الفشل، ENGINE_REGISTRY يبقى فارغاً
    logger.debug("Failed to auto-bootstrap ENGINE_REGISTRY: %s", _bootstrap_err)
