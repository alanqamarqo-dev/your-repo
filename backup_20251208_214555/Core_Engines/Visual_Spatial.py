...
class VisualSpatialEngine(Engine):
    ...
    # make VisualSpatialEngine compatible with engine.process_task(payload)
    def process_task(self, payload: Any) -> Dict[str, Any]:
        # Accept either a dict with 'text'/'scene' or a simple string
        try:
            ...
            # Heuristic: if payload contains spatial keywords, analyze; else describe/generate
            t = (text or '').lower()
            if any(k in t for k in ['فوق', 'تحت', 'داخل', 'خارج', 'بجانب', 'أمام', 'خلف', 'above', 'below', 'inside', 'outside', 'near']):
                return {'ok': True, 'result': self.analyze_spatial_description(text)}
            if any(k in t for k in ['صف', 'وصف', 'مشهد', '3d', 'ثلاثي', 'تخطيط', 'صوّر', 'غرفة']):
                return {'ok': True, 'result': self.describe_or_generate(text)}

            # default lightweight response
            return {'ok': True, 'result': {'text': 'visual_spatial: no detailed action', 'input': text}}
