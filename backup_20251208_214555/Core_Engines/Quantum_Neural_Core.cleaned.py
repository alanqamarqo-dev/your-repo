...
class QuantumNeuralCore:
    ...
    def sample_hypotheses(self, user_query, context=None, num_samples=3):
        ...
        def _extract_best_text(obj):
            ...
            # if looks like Python dict repr, try ast literal eval
            if isinstance(s, str) and s.strip().startswith('{'):
                try:
                    p = ast.literal_eval(s)
                    if isinstance(p, dict):
                        vals = [v for v in p.values() if isinstance(v, str) and v.strip()]
                        if vals:
            ...
            # strip surrounding braces/quotes and key labels if present
            s2 = re.sub(r"^[\{\[]+|[\}\]]+$", "", s).strip()
            s2 = re.sub(r"^['\"]+|['\"]+$", "", s2).strip()
            # remove key: patterns like "key": "value" -> leave value
            s3 = re.sub(r"[\w\-\u0600-\u06FF\u0400-\u04FF]+\s*:\s*", "", s2)
            # collapse whitespace
            candidate = re.sub(r'\s+', ' ', s3).strip()
            return candidate if candidate else s

        try:
            llm = getattr(self, 'llm', None)
            if llm is None and HostedLLMAdapter is not None:
                try:
                    llm = HostedLLMAdapter()
                except Exception:
                    llm = None
            ...
            if llm is not None:
                ...
                try:
                    resp = llm.process_task(task, timeout_s=60)
                except TypeError:
                    resp = llm.process_task(task)

            # normalize response to text blob
            blob = ''
            ...
            return [
                {"id": "fallback_1", "hypothesis": f"Standard Analysis: Investigate root causes of '{user_query}' linearly.", "confidence": 0.5, "support": ["Fallback mechanism triggered."]}
            ]
