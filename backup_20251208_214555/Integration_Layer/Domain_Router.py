...
# --- NEW: Code Generator Import ---
CodeEngineClass = _try_import(
    "Core_Engines.Code_Generator",
    ["Code_Generator", "CodeGeneratorEngine", "CodeGenerator"]
)
...
# --- NEW: Scientific Assistant Import ---
ScienceEngineClass = _try_import(
    "Scientific_Systems.Scientific_Research_Assistant",
    ["Scientific_Research_Assistant", "ScientificResearchAssistant", "ResearchAssistant"]
)
...
_ENGINE_REGISTRY = {
    ...
    "code":     _make(CodeEngineClass),
    "science":  _make(ScienceEngineClass),
}
...
def route_intent(intent: str, payload: dict):
    ...
    if intent == "intent_code":
        if CodeEngineClass is None:
            return {"error": "code_engine_missing", "text": "⚠️ محرك الكود غير موجود."}
        ce = get_engine("code")
        task = payload.get("text") or payload.get("task", "")
        lang = payload.get("language", "python")
        if hasattr(ce, "generate_code"):
            return ce.generate_code(task, language=lang)
        elif hasattr(ce, "write_script"):
            return ce.write_script(task)
        else:
            return getattr(ce, "generate", lambda x: {"text": f"Simulated Code: {x}"})(task)

    # --- NEW: Scientific Research Route ---
    if intent == "intent_science":
        if ScienceEngineClass is None:
            return {"error": "science_engine_missing", "text": "⚠️ محرك العلوم غير موجود."}
        se = get_engine("science")
        topic = payload.get("text") or payload.get("topic", "")
        if hasattr(se, "conduct_research"):
            return se.conduct_research(topic)
        elif hasattr(se, "analyze"):
            return se.analyze(topic)
        else:
            return getattr(se, "research", lambda x: {"text": f"Research on {x} completed."})(topic)
