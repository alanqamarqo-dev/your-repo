    ...
    def process_task(self, payload: dict):
        ...
        try:
            ...
            if isinstance(out, dict):
                return {'ok': True, 'engine': 'ollama', 'text': out.get('answer') or out.get('text')}
...
try:
    _AGL_OLLAMA_TOP_K = int(os.environ.get('AGL_OLLAMA_TOP_K', '3'))
except Exception:
    _AGL_OLLAMA_TOP_K = 3
...
if not hasattr(OllamaKnowledgeEngine, 'process_task'):
    def _ollama_process_task(self, payload: dict) -> dict:
        try:
            ...
            return {'ok': False, 'error': str(e)}
    setattr(OllamaKnowledgeEngine, 'process_task', _ollama_process_task)
