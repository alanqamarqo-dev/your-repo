...
TEST_SOL = (
    ROOT_DIR / "agl_security_tool" / "test_contracts" / "vulnerable" / "vulnerable.sol"
)
...
def cmd_install():
    ...
    if not req_file.exists():
        print_err(f"requirements.txt not found")
        return False
    ...
    try:
        ...
        subprocess.run(
            [sys.executable, "-m", "pip", "install", "z3-solver"], capture_output=True
        )
        try:
            import z3
...
def cmd_check():
    ...
    # Core modules (security tool package)
    core_modules = {
        "core.py": TOOL_DIR / "core.py",
        "z3_symbolic_engine.py": TOOL_DIR / "z3_symbolic_engine.py",
        "exploit_reasoning.py": TOOL_DIR / "exploit_reasoning.py",
        "poc_generator.py": TOOL_DIR / "poc_generator.py",
        "project_scanner.py": TOOL_DIR / "project_scanner.py",
        "risk_core.py": TOOL_DIR / "risk_core.py",
        "solidity_flattener.py": TOOL_DIR / "solidity_flattener.py",
        "detectors/__init__.py": TOOL_DIR / "detectors" / "__init__.py",
        "state_extraction/engine.py": TOOL_DIR / "state_extraction" / "engine.py",
        "audit_pipeline.py": TOOL_DIR / "audit_pipeline.py",
    }
    ...
    try:
        ...
        print_ok("AGLSecurityAudit loads successfully")
    except Exception as e:
        print_err(f"AGLSecurityAudit failed: {e}")
        all_ok = False

    try:
        from agl_security_tool.z3_symbolic_engine import Z3SymbolicEngine

        print_ok("Z3SymbolicEngine loads successfully")
    except Exception as e:
        print_warn(f"Z3SymbolicEngine: {e}")

    try:
        from agl_security_tool.state_extraction import StateExtractionEngine

        print_ok("StateExtractionEngine loads successfully")
    except Exception as e:
        print_warn(f"StateExtractionEngine: {e}")

    try:
        from agl_security_tool.detectors import DetectorRunner

        print_ok("DetectorRunner loads successfully")
    except Exception as e:
        print_err(f"DetectorRunner failed: {e}")
        all_ok = False
...
def cmd_status():
    ...
    engines = [
        ("AGLSecurityAudit", "agl_security_tool.core", "AGLSecurityAudit"),
        (
            "Z3SymbolicEngine",
            "agl_security_tool.z3_symbolic_engine",
            "Z3SymbolicEngine",
        ),
        (
            "StateExtractionEngine",
            "agl_security_tool.state_extraction.engine",
            "StateExtractionEngine",
        ),
        ("DetectorRunner", "agl_security_tool.detectors", "DetectorRunner"),
        (
            "ExploitReasoningEngine",
            "agl_security_tool.exploit_reasoning",
            "ExploitReasoningEngine",
        ),
        ("PoCGenerator", "agl_security_tool.poc_generator", "PoCGenerator"),
        ("RiskCore", "agl_security_tool.risk_core", "RiskCore"),
        ("ProjectScanner", "agl_security_tool.project_scanner", "ProjectScanner"),
        (
            "SolidityFlattener",
            "agl_security_tool.solidity_flattener",
            "SolidityFlattener",
        ),
        (
            "HeikalTunnelingScorer",
            "agl_security_tool.heikal_math.tunneling_scorer",
            "HeikalTunnelingScorer",
        ),
        (
            "WaveDomainEvaluator",
            "agl_security_tool.heikal_math.wave_evaluator",
            "WaveDomainEvaluator",
        ),
        (
            "ActionSpaceBuilder",
            "agl_security_tool.action_space.builder",
            "ActionSpaceBuilder",
        ),
        (
            "AttackSimulationEngine",
            "agl_security_tool.attack_engine.engine",
            "AttackSimulationEngine",
        ),
        (
            "SearchOrchestrator",
            "agl_security_tool.search_engine.engine",
            "SearchOrchestrator",
        ),
        ("WeightOptimizer", "agl_security_tool.weight_optimizer", "WeightOptimizer"),
        ("BenchmarkRunner", "agl_security_tool.benchmark_runner", "BenchmarkRunner"),
    ]
...
def cmd_test():
    ...
    (
        print_ok(f"Quick scan: {findings} findings in {t1-t0:.2f}s")
        if findings > 0
        else print_err("Quick scan: 0 findings!")
    )
    ...
    (
        print_ok(f"Standard scan: {total} findings in {t1-t0:.2f}s")
        if total > 0
        else print_err("Standard scan: 0 findings!")
    )
    ...
    print_ok(
        f"Text: {len(text_report)} chars, MD: {len(md_report)} chars, JSON: {len(json_report)} chars"
    )
...
def cmd_scan(target, format="text", output=None, mode="scan"):
    ...
    print_header(
        f"{'🔬 Deep' if mode == 'deep' else '⚡ Quick' if mode == 'quick' else '🛡️ Standard'} Scan"
    )
    ...
    if command == "info":
        ...
        if stats.get("largest_files"):
            ...
            for f in stats["largest_files"][:5]:
                print(
                    f"     {f['loc']:>5} LOC  {f['file']}  ({', '.join(f['contracts'])})"
                )
...
def cmd_benchmark():
    ...
    print_ok(
        f"Quick scan: avg {avg*1000:.0f}ms ({min(times)*1000:.0f}ms - {max(times)*1000:.0f}ms)"
    )
...
Commands:
  ...
  clean      تنظيف الكاش
    ...
    parser.add_argument(
        "-m",
        "--mode",
        choices=["quick", "scan", "deep"],
        default="scan",
        help="وضع الفحص (لأمر project)",
    )
    parser.add_argument(
        "--include-tests", action="store_true", help="فحص ملفات الاختبار"
    )
    parser.add_argument(
        "--include-deps", action="store_true", help="فحص المكتبات المثبتة"
    )
    parser.add_argument(
        "--include-mocks", action="store_true", help="فحص ملفات المحاكاة"
    )
